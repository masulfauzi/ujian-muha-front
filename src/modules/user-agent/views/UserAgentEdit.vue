<template>
  <div class="bg-surface min-h-screen">
    ...
    <main class="lg:ml-64 min-h-screen">
      ...
      <!-- Page Title -->
      <div class="mb-8">
        <h1 class="text-3xl font-bold text-slate-900">Edit User Agent</h1>
        <p class="text-slate-500 mt-1">Perbarui informasi user agent</p>
      </div>
...
onMounted(async () => {
  try {
    ...
    const userAgent = userAgentStore.selectedUserAgent

    if (userAgent) {
      formData.user_agent = userAgent.user_agent
      formData.x_requested_with = userAgent.x_requested_with || ''
      formData.keterangan = userAgent.keterangan || ''
    }
  } catch (err) {
    error.value = 'User agent tidak ditemukan'
  } finally {
...
const handleSubmit = async () => {
  ...
  try {
    await userAgentStore.updateUserAgent(userAgentId, {
    ...
    error.value = userAgentStore.error || 'Gagal mengupdate user agent'
